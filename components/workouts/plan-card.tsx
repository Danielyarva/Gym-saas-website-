...
import type { WorkoutPlanSummary, WorkoutPlanStatus } from '@/types';

const STATUS_CONFIG: Record<WorkoutPlanStatus, { label: string; variant: 'success' | 'secondary' | 'muted' }> = {
...
export function PlanCard({ plan, selected, onSelect }: PlanCardProps) {
  ...
  return (
    ...
    >
      ...
      {plan.description ? <p className="truncate text-xs text-muted-foreground">{plan.description}</p> : null}
    </button>
  );
}
